...
#           Stop-Exits zusaetzlich 1,0 bps Slippage
# Holdout:  Die juengsten 30 % der Sessions sind QUARANTAENE und werden
#           hier NIE angefasst — sie gehoeren dem einen /colab-Test.
#           Das Quarantaene-Startdatum wird beim Lauf ausgegeben.
# Regeln:   EIN fester Parametersatz pro Idee, kein Sweep, keine Optimierung.
#           Signale nutzen ausschliesslich Information vor dem Einstiegszeitpunkt.
# Laufzeit: erste Ausfuehrung ~5-10 Min (Download ~3.800 Tagesdateien),
#           danach <1 Min dank Cache in /content/dk_cache
# =========================================================================

import warnings, os, io, lzma, struct, time, math, sys
...
R1_THRESH    = 0.004    # S1: |Rendite 09:30-10:00| >= 0,4% ~ halbe mittlere Tagesrange-Haelfte
ORB_TP_R     = 10.0     # S2: Take-Profit bei 10R (Zarattini/Aziz 2023, Originalwert)
LETF_THRESH  = 0.010    # S6: |Tagesrendite bis 15:30| >= 1% loest LETF-Rebalancing-Flow aus
...
print("keine Optimierung — ein fester Parametersatz pro Idee; nur Trainingsfenster,")
    ...
    for versuch in range(3):
        try:
            ...
            if r.status_code == 404 or (r.status_code == 200 and len(r.content) == 0):
                open(fn, "wb").close()   # als leer cachen
                return d, None
...
tage = tage_liste()
print(f"Lade {len(tage)} Handelstage {START} .. {tage[-1]} von Dukascopy ...")
...
#     Die OCLH-Reihenfolge wird unten gegen die OHLC-Alternative verifiziert.
...
for raw in list(rohdaten.values())[:5]:          # robuste Probe ueber mehrere Dateien
    try:
        kand = entpacke(raw, DT_OCLH)
    except lzma.LZMAError:
        continue
    if len(kand) < 100:
        continue
    FORMAT = DT_OCLH if ohlc_ok(kand) > 0.95 else DT_OHLC
    probe = entpacke(raw, FORMAT)
    break
...
# ------------------------------------------------------------------
# 3) SESSION-FRAME — Ankerpreise je US-Handelstag (NY-Zeit)
# ------------------------------------------------------------------
...
S = S.sort_index()
S["prev_close"] = S["close1600"].shift(1)
...
print(f"\n{len(S)} gueltige Sessions. TRAINING: {train.index[0].date()} .. "
...
def s1_intraday_momentum():
    ...
    r1 = train["p1000"] / train["prev_close"] - 1
...
def s2_orb():
    ...
    for d in train.index:
        ...
        if i_stop <= i_tp and i_stop < 10**9:      # Stop zuerst (konservativ)
            g = dir_ * (stop / entry - 1); extra = SLIP
        elif i_tp < 10**9:
...
#      reines Daytrading). Abbildung: Position auf den ersten 3 Sessions des
#      Folgemonats, Kosten einmal pro Fenster.
def s4_tom():
    ret, cost, brutto = leer()
    monat = train.index.to_period("M")
    daily = train["close1600"] / train["prev_close"] - 1
    for p in monat.unique()[:-1]:
        naechste = train.index[monat == p + 1]
        halte = naechste[:3]
        if len(halte) < 3:
            continue
...
def s6_letf():
    ...
    aktiv = (dir_ != 0) & pd.Series(g, index=train.index).notna()
    brutto[aktiv] = g[aktiv]
    cost[aktiv] = COST
    ret[aktiv] = g[aktiv] - COST
    return ret, cost, brutto
...
def s8_margin_rebound():
    ret, cost, brutto = leer()
    prev_ret = (train["close1600"] / train["prev_close"] - 1).shift(1)
    gap = train["open0930"] / train["prev_close"] - 1
    trigger = (prev_ret <= MARGIN_PREV) & (gap <= MARGIN_GAP)
    for d in train.index[trigger.fillna(False)]:
        ...
        stop_hit = np.where(lo[nach] <= stop)[0]
        if len(stop_hit):
            g = stop / entry - 1; extra = SLIP
        else:
...
try:
    ...
    for versuch in range(3):
        df_tlt = yf.download("TLT", start=str(START), auto_adjust=True, progress=False)
        if len(df_tlt) > 500:
            c = df_tlt["Close"]
...
print(f"8 Ideen getestet (12 versucht, 3 starben am Mechanismus-Gate, 1 geparkt) "
